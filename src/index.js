...
var game = new Phaser.Game(720, 720, Phaser.CANVAS, 'phaser-example', { preload: preload, create: create }, true);
...
const request = new Request('./assets/sprites/asset05.png');
...
function preload() {
    ...
    game.load.spritesheet("GEMS", "./assets/sprites/asset05.png", GEM_SIZE, GEM_SIZE);
    game.load.spritesheet("GEMS_ACTIVE", "./assets/sprites/asset03-1.png", GEM_SIZE, GEM_SIZE);
...
function slideGem(pointer, x, y) {
    ...
    //console.log(x);
    //console.log(y);
    if(selectedItem != null && pointer.isUp){
        //alert("isUp");
        for (var i = 0; i < arItemsSelected.length; i++) {
            // Iterate over numeric indexes from 0 to 5, as everyone expects.
            //console.log(arItemsSelected[i]);
            setUnSelected(gems, arItemsSelected[i].x, arItemsSelected[i].y);
            arItemsSelected.splice(i, 1);
    ...
    if (selectedGem && pointer.isDown)
    {
        ...
        //console.log("cursor");
        //console.log(cursorGemPosX);
        //console.log(cursorGemPosY);


        //console.log(getGem(cursorGemPosX, cursorGemPosY));
        //console.log(getGemColor(getGem(cursorGemPosX, cursorGemPosY)));
        //$("#gem").text(getGemColor(getGem(cursorGemPosX, cursorGemPosY)));

        /*//console.log(selectedItem);
        if(selectedItem != null){
            //selectedGem = { x : cursorGemPosX, y : cursorGemPosY };
            if(selectedItem == getGemColor(getGem(cursorGemPosX, cursorGemPosY))){
                //console.log({ x : cursorGemPosX, y : cursorGemPosY });
                setSelected(gems, cursorGemPosX, cursorGemPosY);
                arItemsSelected.push({ x : cursorGemPosX, y : cursorGemPosY });
                $("#gem").text(arItemsSelected.length);

            }
        }else{
            selectedItem = getGemColor(getGem(cursorGemPosX, cursorGemPosY));
        }

        //console.log("isDown",pointer.isDown);
        //console.log("isUp",pointer.isUp);
        */
...
function setSelected(gems,x,y){
    //gem1 is a Sprite
    var gem1 = gems.create(x * GEM_SIZE_SPACED, y * GEM_SIZE_SPACED, "GEMS_ACTIVE");
    gem1.name = 'gem' + x.toString() + 'x' + y.toString();
    gem1.inputEnabled = true;
    gem1.events.onInputDown.add(selectGem, this);
    gem1.events.onInputUp.add(releaseGem, this);
    gem1.frame = getGemColor(getGem(x, y));

    getGem(x, y).frame = gem1;
    //setGemPos(gem1, x, y);
    //gem1.kill();
}

function setUnSelected(gems,x,y){
    var gem1 = gems.create(x * GEM_SIZE_SPACED, y * GEM_SIZE_SPACED, "GEMS");
    gem1.name = 'gem' + x.toString() + 'x' + y.toString();
    gem1.inputEnabled = true;
    gem1.events.onInputDown.add(selectGem, this);
    gem1.events.onInputUp.add(releaseGem, this);
...
function removeKilledGems() {
    ...
    gems.forEach(function(gem) {
        ...
        if (!gem.alive) {
            if(items >= 0) {
                ...
                var fname = ("#f"+parseInt(gem.frame).toString());

                if(parseInt($("#f"+gem.frame).text()) > 0)$(fname).text(parseInt($("#f"+gem.frame).text())-1);
                ini++;
            }
    ...
    if(ini>0)  {
        ...
        if(parseInt($("#moves").text())>0){
            $("#moves").text(parseInt($("#moves").text())-1);


            if(parseInt($("#moves").text())<0){
                if(parseInt($("#f1"))>0||parseInt($("#f2"))>0||parseInt($("#f3"))>0||parseInt($("#f4"))>0||parseInt($("#f0"))>0){
                    $( "#dialog" ).dialog("open");
                }else{
                    $( "#dialog2" ).dialog( "open" );
                }
